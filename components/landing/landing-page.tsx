...
export function LandingPage() {
  return (
    <div className="flex h-dvh max-h-dvh flex-col overflow-hidden bg-background text-foreground">
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-border/40 px-safe py-3 pt-[max(0.75rem,env(safe-area-inset-top,0px))]">
      ...
      >
        ...
        <p className="mt-2 max-w-lg text-pretty text-xs leading-snug text-muted-foreground sm:mt-3 sm:text-sm">
        ...
        <ul className="mt-3 max-w-lg space-y-1 text-[11px] leading-snug text-muted-foreground sm:mt-4 sm:space-y-1.5 sm:text-sm">
        ...
        <p className="mt-3 max-w-lg text-[10px] leading-snug text-muted-foreground sm:text-xs">
          <strong className="text-foreground">18+</strong>. Entertainment only — not medical advice.
        </p>
        <div className="mt-4 flex max-w-lg flex-col gap-1.5 sm:mt-5 sm:gap-2">
      ...
      <footer className="shrink-0 border-t border-border/40 px-safe py-1.5 pb-[max(0.5rem,env(safe-area-inset-bottom,0px))] text-center text-[10px] leading-tight text-muted-foreground">
        Beta chat lobby · races optional · not clinical care
      </footer>
    </div>
  );
}

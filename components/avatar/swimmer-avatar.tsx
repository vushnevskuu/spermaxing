...
function FlagellumTail(props: {
  ...
  const { tailType, tailColor, strokeWidth, moving, referenceLobbyStyle, size } = props;
  ...
  const amp = moving ? p.ampMove : p.ampIdle;
  const dur = referenceLobbyStyle
    ? moving
      ? p.durMove * 1.08
      : p.durIdle * 0.92
    : moving
  ...
  const baseW = (strokeWidth + 0.8) * (p.strokeBoost ?? 1);
        ...
        >
          ...
          >
            ...
            <ellipse
              ...
              strokeWidth={stroke}
            />
            <ellipse cx="34" cy="19" rx="5" ry="6" fill="#fff" />
            <ellipse cx="48" cy="19" rx="5" ry="6" fill="#fff" />
